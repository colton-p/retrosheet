...
class Play:
    ...
    def from_play_line(rec: EventRecords.Play):
        ...
        return Play('', int(rec.inning), int(rec.home_ind), rec.player, rec.count, rec.pitch_seq, outcome, modifier, advances)


    def classify(self):
        attrs = ['reach_on_error', 'h', 'bb', 'k', 'hbp', 'bip_out', 'fc', 'fle', 'catchers_int']
    ...
    @property
    def k(self):
        return re.fullmatch(r'K[1-9]{0,4}', self.outcome) or self.outcome.startswith('K+')
    ...
    @property
    def ab(self):
        if self.sf or self.sh: return False
